...
/**
 * Created by dbaskette on 3/23/17.
 */
@Controller
public class HomeController {
    ...
    @Autowired
    public HomeController(ImageService imageService,SimpMessagingTemplate messagingTemplate){
    ...
    public String deleteFile(@PathVariable String filename, RedirectAttributes redirectAttributes){
        try {
            imageService.deleteImage(filename,true);
    ...
    public String index(Model model, Pageable pageable){
        final Page<UploadedImage> page = imageService.findPage(pageable);
